async function handler(m, { conn, usedPrefix, command, text }) {
    ...
    if (SWORD || ARMOR || HEALT) {
        ...
        if (SWORD) message += `${prefix}craft sword - Membuat Sword\n`;
        if (ARMOR) message += `${prefix}craft armor - Membuat Armor\n`;
    ...
    if (room) {
        ...
        if (room.game.player1 && room.game.player2 && room.game.player3 && room.game.player4) {
            ...
            room.price.uang += (Math.floor(Math.random() * 1202015)) * 1
            room.price.exp += (Math.floor(Math.random() * 1140186)) * 1
            room.price.iron += (pickRandom([10, 7, 2, 10, 8, 1, 5, 0])) * 1
            room.game.diamond += (pickRandom([5, 10, 10, 0, 1, 8, 5, 0, 0, 5, 18, 16, 20, 10, 0])) * 1
            room.game.sampah += (Math.floor(Math.random() * 600)) * 1
            room.price.string += (Math.floor(Math.random() * 100)) * 1
            room.price.kayu += (Math.floor(Math.random() * 200)) * 1
            room.price.batu += (Math.floor(Math.random() * 200)) * 1
            room.game.makananPet += (pickRandom([20, 0, 10, 7, 10, 0, 4, 6, 5, 0, 7, 19])) * 1
            room.game.common += (pickRandom([10, 6, 0, 15, 0, 6, 9, 2, 9, 0])) * 1
            room.game.uncommon += (pickRandom([18, 19, 7, 15, 8, 0, 1, 0, 20, 0, 6, 10])) * 1
...
*🗑️sampah:* ${sampah  * 4}${potion == 0 ? '' : '\n*🥤Potion:* ' + potion * 4}${makananPet == 0 ? '' : '\n*🍖Makanan Pet* ' + makananPet * 4}${kayu == 0 ? '' : '\n*🪵Kayu:* ' + kayu * 4}${batu == 0 ? '' : '\n*🪨Batu:* ' + batu * 4}${string == 0 ? '' : '\n*🕸️String:* ' + string * 4}${iron == 0 ? '' : '\n*⛓️Iron:* ' + iron * 4}${diamond == 0 ? '' : '\n*💎diamond:* ' + diamond * 4}${common == 0 ? '' : '\n*📦common crate:* ' + common * 4}${uncommon == 0 ? '' : '\n*📦uncommon crate:* ' + uncommon * 4}
             `.trim()
                ...
                if ((_H1 || _H2 || _H3 || _H4 || _sd1 || _sd2 || _sd3 || _sd4) < 1) {
                    ...
                    let str3 = `${((SDH || SDH.length > 0) || (SDM1L || SDM1L.length > 0)) ? `⚔️Sword ${((SDH || SDH.length > 0 ? sI + ' Hancur, silahkan beli ⚔️Sword kembali dengan mengetik *' + usedPrefix + 'craft sword*' : '') + (SDM1L || SDM1L.length > 0 ? (SDH || SDH.length > 0 ? ', Sedangkan ⚔️Sword ' : '') + sH + ' Hancur, dan Menurun *1* Level' : ''))}` : ''}${HEALT || healt.length > 0 ? `❤️Nyawa ${H} habis, silahkan isi ❤️Nyawa dengan mengetik ${usedPrefix}heal` : ''}`
                    await m.reply(str3, c1, {
                        contextInfo: {
                            mentionedJid: conn.parseMention(str3)
                        }
        ...
        room = {
            ...
            price: {
                uang: (Math.floor(Math.random() * 100000)) * 1,
                exp: (Math.floor(Math.random() * 70100)) * 1,
                sampah: (Math.floor(Math.random() * 200)) * 1,
                potion: (Math.floor(Math.random() * 40)) * 1,
                diamond: (pickRandom([0, 20, 10, 11, 15, 1, 0])) * 1,
                iron: (Math.floor(Math.random() * 100)) * 1,
                kayu: (Math.floor(Math.random() * 200)) * 1,
                batu: (Math.floor(Math.random() * 200)) * 1,
                string: (Math.floor(Math.random() * 200)) * 1,
                common: (pickRandom([16, 9, 8, 1, 0, 18])) * 1,
                uncommon: (pickRandom([18, 9, 4, 0, 1, 8, 0])) * 1,
                mythic: (pickRandom([0, 5, 1, 4, 10, 0, 7, 0, 3])) * 1,
                legendary: (pickRandom([8, 19, 7, 9, 0, 1, 1, 6, 5, 4, 0])) * 1,
                pet: (pickRandom([10, 6, 0, 1, 4, 7, 4, 0, 1, 0])) * 1,
                makananPet: (pickRandom([1, 12, 0, 1, 8, 10, 0, 0])) * 1,
            },
            less: {
...
`.trim()
                ...
                if ((users.healt * 1) < 1 || (users.sworddurability * 1) < 1) {
                    ...
                    let str3 = `${__sword1 ? `⚔️Sword Kamu ${_sword1 ? ` Level nya berkurang 1 karena hancur` : ` Hancur, dan silahkan beli ⚔️Sword kembali dengan mengetik ${usedPrefix}`}craft sword` : ''} ${healt1 ? `${__sword1 ? 'Dan ' : ''}❤️Nyawa Kamu habis, silahkan isi kembali dengan ketik ${usedPrefix}heal` : ''}`
                    m.reply(str3, room.player1, {
                        contextInfo: {
...
`.trim()
            ...
            for (let _p of PLAYER) {
                room.price.uang += (Math.floor(Math.random() * 41000)) * 1
                room.price.exp += (Math.floor(Math.random() * 76000)) * 1
                room.game.sampah += (Math.floor(Math.random() * 300)) * 1
                room.price.string += (pickRandom([ 8, 10, 15, 0, 5, 0, 20])) * 1
                room.price.kayu += (pickRandom([90, 60, 20, 20, 30, 100, 50, 0])) * 1
                room.price.batu += (pickRandom([80, 0, 7, 100, 7, 40, 8, 60, 30, 0])) * 1
                room.game.common += (pickRandom([60, 20, 20, 30, 100, 50, 110, 0, 80])) * 1
            }
