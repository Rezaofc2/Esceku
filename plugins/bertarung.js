let handler = async (m, { conn, text }) => {
 ...
 let betAmount = Math.floor(Math.random() * (10000 - 1000 + 1)) + 1000
  
  if (user.uang < betAmount) {
  ...
  setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        user.uang += wonAmount
        global.db.data.users[opponent].uang -= wonAmount
        
        let opponentName = conn.getName(opponent) 
        
        let caption = `❏  *F I G H T*\n\n`
        caption += `Lawan Anda Adalah: ${global.db.data.users[opponent].name}\nLevel: [${global.db.data.users[opponent].level}]\n\n`
        
        if (result) {
          caption += `*Menang!*, ${alasanMenang},+${betAmount} Money\n`
          caption += `Uang Anda saat ini: ${user.uang}\n`
