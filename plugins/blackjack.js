...
export async function handler(m, { conn, usedPrefix, command, text }) {
  ...
  try {
    ...
    if (!(m.sender in conn.bj)) {
      ...
      if (isNaN(bet) || 1000001 <= bet ) {
        return m.reply('*Total money yang ingin dipertaruhkan harus lebih dari atau sama dengan 1000000!*');
      }
      if (global.db.data.users[m.sender].uang < bet) {
      ...
      if (playerTotal === 21 && computerTotal !== 21) {
        ...
        global.db.data.users[m.sender].uang += bet * 1;
        ...
        conn.bj[m.sender] = {
          ...
          bet: bet,
          timeout: setTimeout(() => {
            m.reply(`waktu habis -${bet}`);
            global.db.data.users[m.sender].uang -= bet
            delete conn.bj[m.sender];
          }, 60000)
        };
