 ...
 let timers = clockString(_timers)
  ...
  if (new Date - global.db.data.users[m.sender].lasthunt > 1200000) {
   ...
   let dmg = (player.sword  * 5 + player.armor * 5 - sum)
    ...
    if (player.healt < 0) {
      let msg = `*@${pengirim}* Anda Mati Di Bunuh Oleh ${monsterName} nLevel Anda Turun 1 Karena Mati Saat Berburu!\nSword Anda Berkurang 5 Karena Mati Saat Berburu!`
      if (player.level > 0) {
      if (player.sword > 0) {
        player.level -= 1
        player.sword -= 5
      ...
      conn.reply(m.chat, msg, m, { contextInfo: { externalAdReply: {title: wm, body: null, sourceUrl: yt, thumbnailUrl: pp }}})
      return
    }
...
+1 Tiketcoin`
   conn.reply(m.chat, pesan, m, { contextInfo: { externalAdReply: {title: wm, body: null, sourceUrl: yt, thumbnailUrl: pp }}})
