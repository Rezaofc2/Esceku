const timeout = 1800000
                                     ...
                                     if (global.db.data.users[m.sender].bibitmangga > 2) {
                                 	if (global.db.data.users[m.sender].bibitapel > 2) {
                                 	if (global.db.data.users[m.sender].bibitpisang > 2) {
                                 	if (global.db.data.users[m.sender].bibitjeruk > 2) {
                                 	if (global.db.data.users[m.sender].bibitanggur > 2) {
                                     ...
                                     let apelpoin = `${Math.floor(Math.random() * 60)}`.trim()
                                     ...
                                     global.db.data.users[m.sender].bibitpisang -= 2
                                     global.db.data.users[m.sender].bibitanggur -= 2
                                     global.db.data.users[m.sender].bibitmangga -= 2
                                     global.db.data.users[m.sender].bibitjeruk -= 2
                                     global.db.data.users[m.sender].bibitapel -= 2
                                     global.db.data.users[m.sender].lastberkebon = new Date * 1
                                     m.reply(`Selamat kamu mendapatkan : \n+${pisangpoin} Pisang\n+${manggapoin} Mangga\n+${anggurpoin} Anggur\n+${jerukpoin} Jeruk\n+${apelpoin} Apel\n+1 Tiketcoin`)
                                     setTimeout(() => {
					                      conn.reply(m.chat, `Waktunya berkebon lagi kak 😅`, m)
					                  }, timeout)
                                  } else m.reply(`Pastikan bibit anggur kamu *2* untuk bisa berkebon`)
                              } else m.reply(`Pastikan bibit jeruk kamu *2* untuk bisa berkebon`)
                          } else m.reply(`Pastikan bibit pisang kamu *2* untuk bisa berkebon`)
                      } else m.reply(`Pastikan bibit apel kamu *2* untuk bisa berkebon`)
                  } else m.reply(`Pastikan bibit mangga kamu *2* untuk bisa berkebon`)
              }
