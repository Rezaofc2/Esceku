let handler = async (m, { conn, text }) => {
  ...
  if (global.db.data.users[user].pasangan == "") {
    conn.reply(m.chat, `*kamu tidak memiliki pasangan dan tidak sedang menembak siapapun*\n\n*Ketik /tembak @user untuk menembak seseorang*`, m)
  }else if (global.db.data.users[global.db.data.users[user].pasangan].pasangan != user){
    conn.reply(m.chat, `*kamu sedang digantung oleh @${global.db.data.users[user].pasangan.split('@')[0]} karena sedang tidak diterima atau di tolak*\n\n*Ketik .ikhlasin untuk menghapus nama dia dari hatimu*`, m,{contextInfo: {
      mentionedJid: [global.db.data.users[user].pasangan]
    }})
  }else {
    conn.reply(m.chat, `*kamu sedang menjalani hubungan dengan @${global.db.data.users[user].pasangan.split('@')[0]} 🥳🥳*`, m,{contextInfo: {
      mentionedJid: [global.db.data.users[user].pasangan]
    }})
  }
}
