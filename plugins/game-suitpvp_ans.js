...
handler.before = async function (m) {
  ...
  if (room) {
    ...
    if (m.sender == room.p2 && /^(acc(ept)?|terima|gas|oke?|tolak|gamau|nanti|ga(k.)?bisa)/i.test(m.text) && m.isGroup && room.status == 'wait') {
      ...
      let caption = `Suit telah dikirimkan ke chat
@${room.p.split`@`[0]} dan 
@${room.p2.split`@`[0]}

Silahkan pilih suit di chat masing"
klik wa.me/${conn.user.jid.split`@`[0]}`
conn.reply(m.chat, caption, m, { contextInfo: { mentionedJid: conn.parseMention(caption),forwardingScore: 256,
      ...
      room.waktu_milih = setTimeout(() => {
        if (!room.pilih && !room.pilih2) return this.reply(m.chat, `Kedua pemain tidak niat main,\nSuit dibatalkan`, m)
          ...
          let yo = `@${(room.pilih ? room.p2 : room.p).split`@`[0]} tidak memilih suit, game berakhir`
          this.reply(m.chat, yo, m, { contextInfo: { mentionedJid: this.parseMention(yo),forwardingScore: 256,
      isForwarded: true, externalAdReply: { title: null, body: wm, sourceUrl: yt, thumbnailUrl: pickRandom(hwaifu) }}})
...
`.trim()
this.reply(room.asal, oh, m, { contextInfo: { mentionedJid: this.parseMention(oh),forwardingScore: 256,
      isForwarded: true, externalAdReply: { title: null, body: wm, sourceUrl: yt, thumbnailUrl: pickRandom(hwaifu) }}})
      if (!tie) {
        db.data.users[win == room.p ? room.p : room.p2].exp += room.poin
        db.data.users[win == room.p ? room.p2 : room.p].exp += room.poin_lose
