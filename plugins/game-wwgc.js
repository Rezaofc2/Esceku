    ...
    if (value === "create") {
        ...
        if (ww[chat].status === true)
            return m.reply("Sesi permainan telah dimulai");
             let y = `Hanya @${ww[chat].owner.split("@")[0]} yang dapat memulai permainan`
        if (ww[chat].owner !== sender)
            return conn.reply(m.chat, y, m, { contextInfo: { mentionedJid: conn.parseMention(y),forwardingScore: 256,
      isForwarded: true, externalAdReply: { title: null, body: wm, sourceUrl: yt, thumbnailUrl: pickRandom(hwaifu) }}})
        ...
        conn.sendMessage(m.chat, { text: `@${sender.split("@")[0]} keluar dari permainan`, mentions:  [sender] }, )
        playerExit(chat, sender, ww);
    } else if (value === "delete") {
        if (!ww[chat]) return m.reply("Tidak ada sesi permainan");
         let n = `Hanya @${ww[chat].owner.split("@")[0]} yang dapat menghapus permainan`
        if (ww[chat].owner !== sender)
            return conn.reply(m.chat, n, m, { contextInfo: { mentionedJid: conn.parseMention(n),forwardingScore: 256,
      isForwarded: true, externalAdReply: { title: null, body: wm, sourceUrl: yt, thumbnailUrl: pickRandom(hwaifu) }}})
