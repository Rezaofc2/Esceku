...
*${usedPrefix + command} start* = start game`)
  ...
  if (args[0] == "join"){
    ...
    if (global.db.data.users[m.sender].uang < 1000) return m.reply("*Uang kamu minimal Rp. 1000 untuk bermain game ini.*")
    // FIRST PLAYER
    if (!(m.chat in conn.war)) {
      ...
      if (args[1]){
        if (args[1].toLowerCase() == "a"){
          if (conn.war2[m.chat].uang == 0) return conn.reply(m.chat,`*Tolong @${conn.war[m.chat][0].user.split('@')[0]} tetapkan modal awal perang (minimal Rp. 1.000.000)*\n\n.war money 1000000`,m, {contextInfo : {mentionedJid : [conn.war[m.chat][0].user]}})
          return m.reply('a')
          if (global.db.data.users[m.sender].uang < conn.war2[m.chat].uang) return m.reply(`*Uang kamu minimal Rp. ${conn.war2[m.chat].uang.toLocaleString()} untuk bermain game ini.*`)
          for (let i = 1 ; i < 5 ; i++) {
            if (conn.war[m.chat][i].user == ""){
              let exp = global.db.data.users[m.sender].exp
          ...
          if (conn.war2[m.chat].uang == 0) return conn.reply(m.chat,`*Tolong @${conn.war[m.chat][0].user.split('@')[0]} tetapkan modal awal perang (minimal Rp. 1000000)*\n\n.war money 1000000`,m, {contextInfo : {mentionedJid : [conn.war[m.chat][0].user]}})
          if (global.db.data.users[m.sender].uang < conn.war2[m.chat].uang) return m.reply(`*Uang kamu minimal Rp. ${conn.war2[m.chat].uang.toLocaleString()} untuk bermain game ini.*`)
          for (let i = 5 ; i < 10 ; i++) {
            if (conn.war[m.chat][i].user == ""){
              let exp = global.db.data.users[m.sender].exp
