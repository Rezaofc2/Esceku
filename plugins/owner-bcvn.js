...
const frez = {
			...
			fileLength: 1000, seconds: 10000,
				contextInfo: {
					externalAdReply: {
						...
						thumbnailUrl: pickRandom(flaaa) + 'Broadcast+Audio',
						sourceUrl: null
					}
				}
			}
 let q = m.quoted ? m.quoted : m
  ...
  if (!/audio/.test(mime)) {
    return conn.reply(m.chat, `Balas vn/audio dengan caption *${usedPrefix + command}*`, m);
  }
 ...
 let audio = await q.download()
  ...
  for (const id of groups) {
    ...
    await conn.sendFile(id, audio, null, text, frez, null, fakeig).catch(_ => _);
  }
