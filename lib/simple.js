...
`.trim()
  ...
  sendCarousel: { 
  async value(jid, messages, quoted, json, options = {}) {
    ...
    sendList: { 
    async value(jid, name, array, quoted, json = {}) {
  ...
  sendButtons: { 
  async value(jid, array, quoted, json = {}) {
    const result = [];
...
`.trim()
        ...
        reply: {
            ...
            value(jid, text = '', quoted, options) {
                ...
                return Buffer.isBuffer(text) ? conn.sendFile(jid, text, 'file', '', quoted, false, options) : conn.sendMessage(jid, { text: text, 
                mentions: conn.parseMention(text),
